...
import BinOperation from "./AST/BinOperation";
import UnarOperator from "./AST/UnarOperator";
...
export default class Parser {
    ...
    parsePrint(): ExpressionNode {
        ...
        if (operatorLog != null) {
            return new UnarOperator(operatorLog, this.parseFormula())
        }
    ...
    parseFormula(): ExpressionNode {
        ...
        let operator = this.match(tokenTypeList.MINUS, tokenTypeList.PLUS, tokenTypeList.MULTIPLY, tokenTypeList.DIVISION, tokenTypeList.DEGREE);
        while (operator != null) {
            const rightNode = this.parseParentheses();
            leftNode = new BinOperation(operator, leftNode, rightNode);
            operator = this.match(tokenTypeList.MINUS, tokenTypeList.PLUS, tokenTypeList.MULTIPLY, tokenTypeList.DIVISION, tokenTypeList.DEGREE);
        }
        return leftNode;
    }
    ...
    parseExpression(): ExpressionNode {
        ...
        if (assignOperator != null) {
            ...
            const binaryNode = new BinOperation(assignOperator, variableNode, rightFormulaNode);
            return binaryNode;
        }
    ...
    run(node: ExpressionNode): any {
        ...
        if (node instanceof BinOperation) {
            switch (node.operator.type.name) {
                ...
                case tokenTypeList.MULTIPLY.name:
                    return this.run(node.leftNode) * this.run(node.rightNode)
                case tokenTypeList.DIVISION.name:
                    return this.run(node.leftNode) / this.run(node.rightNode)
                case tokenTypeList.DEGREE.name:
                    return this.run(node.leftNode) ** this.run(node.rightNode)
